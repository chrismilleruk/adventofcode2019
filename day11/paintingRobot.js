...
async function paintingRobot(programBuffer, onEvent) {
  const panels = initPanels();

  for await (const args of paintingRobotEventGenerator(panels, programBuffer)) {
    if (typeof onEvent === "function") onEvent.apply(panels, args);
  }

  return panels;
}

async function* paintingRobotEventGenerator(panels, programBuffer) {
  /* 
  ...
  */
  ...
  while (!iterator.done) {
    let panel = panels.getPanel(state.coord);
...
module.exports = { paintingRobot, paintingRobotEventGenerator, initPanels};
