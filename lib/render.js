...
function renderAllPanels(writeStream, panels, yAxis) {
  ...
  // Plot all panels
  plotPanelsSync(renderer, panels, offset, yAxis);
...
function preparePlotArea(writeStream, width, height, yAxis = YAXIS.TOP_TO_BOTTOM) {
  ...
  let cursor = {
    ...
    log: (...args) => {
      // Write any console output to the right of the border.
      // e.g.  ┗257 ms━━━━━━━━━━━━━━━━┛ |
      //     The cursor is starts here ↗️
      cursor._moveToInternal(this.width + 3, this.logRow);
 ...
 * @param {YAXIS} yAxis
 */
async function plotPanels(cursor, panelsAsync, offset, yAxis) {
  const blocks = new Map();

  for await (const panel of panelsAsync) {
    plotPanelAsBlock(cursor, panel[1], blocks, { offset, yAxis });
  }
}
 ...
 * @param {YAXIS} yAxis
 */
function plotPanelsSync(cursor, panels, offset, yAxis) {
  const blocks = new Map();
 ...
 * @param {{offset:{x: Number, y: Number}, color:{colorFn:Function, when:any}, yAxis:YAXIS}} opts 
 */
function plotPanelAsBlock(cursor, panel, blocks, opts = { offset: { x: 0, y: 0 }, color: { colorFn: chalk.white, when: 1 }, yAxis: YAXIS.TOP_TO_BOTTOM }) {
  let { offset, color, yAxis } = opts;
  offset = offset || { x: 0, y: 0 };
  color = color || { colorFn: chalk.white, when: 1 };
  yAxis = yAxis || YAXIS.TOP_TO_BOTTOM;

  let [coordX, coordY] = panel.coord.slice();
  coordX += offset.x;
  coordY += offset.y;
  const plotCoordX = Math.floor(coordX / 2);
  const plotCoordY = Math.floor(coordY / 2);
  const blockCoordX = Math.abs(coordX) % 2;
  let blockCoordY = Math.abs(coordY) % 2;
  if (YAXIS.BOTTOM_TO_TOP === yAxis) {
    blockCoordY = blockCoordY === 1 ? 0 : 1;
  }
  ...
  block[blockCoordY][blockCoordX] = (panel.color === color.when) ? 1 : 0;

  // Re-render this block.
  cursor.moveTo(plotCoordX, plotCoordY);
  cursor.write(getBlockChar(block), color.colorFn);
}
